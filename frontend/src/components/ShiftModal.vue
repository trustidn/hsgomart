<template>
  <div class="fixed inset-0 z-20 flex items-center justify-center bg-black/50 p-4" @click.self="$emit('close')">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-sm p-6">
      <h2 class="text-lg font-semibold text-gray-800 mb-4">{{ mode === 'open' ? 'Open Shift' : 'Close Shift' }}</h2>

      <form v-if="mode === 'open'" @submit.prevent="submitOpen">
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700 mb-1">Opening cash (IDR)</label>
          <input
        ...
        <div class="flex gap-2 justify-end">
          <button type="button" class="px-3 py-2 text-gray-600 hover:bg-gray-100 rounded-md" @click="$emit('close')">Cancel</button>
      ...
      <template v-else-if="mode === 'close'">
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700 mb-1">Closing cash (IDR)</label>
          <input
        ...
        <div v-if="closeResult" class="mb-4 p-3 bg-gray-50 rounded text-sm">
        ...
        <div class="flex gap-2 justify-end">
          <button type="button" class="px-3 py-2 text-gray-600 hover:bg-gray-100 rounded-md" @click="$emit('close')">{{ closeResult ? 'Done' : 'Cancel' }}</button>
