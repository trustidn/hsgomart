<template>
  <div>
    <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-200 mb-4">Inventory History</h1>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-lg shadow border border-gray-200 dark:border-gray-800 p-4 mb-4">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
        ...
        <div>
          ...
          <select v-model="filters.type" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
            <option value="">Semua</option>
        ...
        <div>
          ...
          <input v-model="filters.from_date" type="date" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Sampai tanggal</label>
          <input v-model="filters.to_date" type="date" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" />
        </div>
        <div class="flex gap-2">
          <button type="button" class="px-3 py-2 bg-slate-600 text-white rounded-md text-sm hover:bg-slate-700" @click="applyFilters">
            Terapkan
          </button>
          <button type="button" class="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300" @click="resetFilters">
            Reset
          </button>
        </div>
      </div>
    ...
    <div v-else class="bg-white dark:bg-gray-900 rounded-lg shadow border border-gray-200 dark:border-gray-800 overflow-hidden divide-y divide-gray-200 dark:divide-gray-700">
      ...
      <div v-if="total > 0" class="px-4 py-3 flex flex-wrap items-center justify-between gap-2 border-t border-gray-200 bg-gray-50 dark:bg-gray-800">
        ...
        <div class="flex items-center gap-2">
          <select v-model.number="pageSize" class="text-sm border border-gray-300 dark:border-gray-600 rounded px-2 py-1 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" @change="goToPage(1)">
          ...
          <button
            type="button"
            class="px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded hover:bg-gray-100 dark:hover:bg-gray-800 disabled:opacity-50 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
            :disabled="page <= 1"
...
function typeBadgeClass(type) {
  ...
  if (t === 'purchase') return 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400'
  if (t === 'sale') return 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400'
  if (t === 'adjustment') return 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-400'
