<template>
  <div>
    <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-6">Revenue Report</h1>

    <!-- Filters -->
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4 mb-6 flex flex-wrap items-end gap-4">
      <div>
        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">From</label>
        <input v-model="fromDate" type="date" class="border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" />
      </div>
      <div>
        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">To</label>
        <input v-model="toDate" type="date" class="border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" />
      </div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
          ...
          <p class="text-2xl font-bold text-gray-800 dark:text-gray-200">Rp {{ Number(report.total_revenue || 0).toLocaleString('id-ID') }}</p>
        </div>
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
          <p class="text-sm text-gray-500 dark:text-gray-400">Total Orders</p>
          <p class="text-2xl font-bold text-gray-800 dark:text-gray-200">{{ report.total_orders || 0 }}</p>
        </div>
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
          <p class="text-sm text-gray-500 dark:text-gray-400">Average per Order</p>
          <p class="text-2xl font-bold text-gray-800 dark:text-gray-200">Rp {{ Number(report.avg_per_order || 0).toLocaleString('id-ID') }}</p>
        </div>
      </div>

      <!-- Plan Breakdown -->
      <div v-if="report.plan_breakdown?.length" class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 mb-6">
        <h2 class="text-base font-semibold text-gray-800 dark:text-gray-200 mb-3">Revenue by Plan</h2>
