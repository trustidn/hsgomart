<template>
  <div>
    ...
    <div v-if="loading" class="text-gray-500 py-8 text-center">Loading...</div>
    ...
    <div v-else class="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="op in opnames" :key="op.id" class="hover:bg-gray-50">
            <td class="px-4 py-3 text-gray-800">{{ formatDate(op.created_at) }}</td>
      ...
      <div v-if="modal" class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" @click.self="closeModal">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
          ...
          <div class="flex-1 overflow-auto px-6 py-4">
            <div v-if="loadingDetail" class="py-8 text-center text-gray-400">Memuat produk...</div>
            <table v-else class="w-full text-sm">
              <thead class="bg-gray-50 sticky top-0">
                <tr>
                  <th class="px-3 py-2.5 text-left text-gray-600 text-xs uppercase">Produk</th>
                  <th class="px-3 py-2.5 text-right text-gray-600 text-xs uppercase w-24">Stok Sistem</th>
                  <th class="px-3 py-2.5 text-right text-gray-600 text-xs uppercase w-28">
                    {{ modal.opname.status === 'draft' ? 'Stok Aktual' : 'Aktual' }}
                  </th>
                  <th class="px-3 py-2.5 text-right text-gray-600 text-xs uppercase w-20">Selisih</th>
                  <th class="px-3 py-2.5 text-left text-gray-600 text-xs uppercase w-40">Catatan</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100">
                <tr v-for="item in filteredItems" :key="item.product_id" class="hover:bg-gray-50">
                  ...
                  <td class="px-3 py-2 text-right">
                    <input v-if="modal.opname.status === 'draft'"
                      ...
                      class="w-20 border border-gray-300 rounded px-2 py-1 text-sm text-right focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"
                  ...
                  <td class="px-3 py-2">
                    <input v-if="modal.opname.status === 'draft'"
                      ...
                      class="w-full border border-gray-300 rounded px-2 py-1 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none" />
                    <span v-else class="text-gray-500 text-xs">{{ item.notes || '—' }}</span>
                  </td>
                </tr>
                <tr v-if="!filteredItems.length">
                  <td colspan="5" class="px-3 py-6 text-center text-gray-400">Tidak ada produk ditemukan</td>
                </tr>
              </tbody>
            </table>
          </div>
