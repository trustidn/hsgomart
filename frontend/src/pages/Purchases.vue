  ...
  <div>
    ...
    <template v-if="!detailId">
      <div class="flex items-center justify-between mb-4">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-200">Purchases</h1>
        <button
          type="button"
      ...
      >
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-2xl border border-gray-200 dark:border-gray-800 max-h-[90vh] overflow-hidden flex flex-col">
          <div class="p-4 border-b border-gray-200 dark:border-gray-700">
            <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-200">Create Purchase</h2>
          </div>
          <form class="flex-1 overflow-auto p-4" @submit.prevent="submitPurchase">
            <div class="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input v-model="form.supplier_name" type="text" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" placeholder="e.g. PT Indofood" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Invoice number</label>
                <input v-model="form.invoice_number" type="text" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" placeholder="e.g. INV-001" />
              </div>
            </div>
            <div class="mb-4">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Keterangan</label>
              <textarea v-model="form.notes" rows="2" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" placeholder="Informasi tambahan (opsional)" />
            </div>
            ...
            <table class="min-w-full border border-gray-200 dark:border-gray-700 rounded overflow-hidden mb-4">
              <thead class="bg-gray-50 dark:bg-gray-800">
                <tr>
                  ...
                  <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase w-24">Qty</th>
                  <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase w-32">Cost price</th>
                  <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase w-32">Subtotal</th>
                  <th class="w-10"></th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                <tr v-for="(row, i) in form.items" :key="i">
                  <td class="px-3 py-2">
                    <select v-model="row.product_id" required class="w-full px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" @change="onProductChange(i)">
                      <option value="">Select product</option>
                      <option v-for="prod in products" :key="prod.id" :value="prod.id">{{ prod.name }}</option>
                    </select>
                  </td>
                  <td class="px-3 py-2 text-right">
                    <input v-model.number="row.quantity" type="number" min="1" class="w-full px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-right" @input="updateSubtotal(i)" />
                  </td>
                  <td class="px-3 py-2 text-right">
                    <input v-model.number="row.cost_price" type="number" min="0" step="0.01" class="w-full px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-right" @input="updateSubtotal(i)" />
                  </td>
                  <td class="px-3 py-2 text-sm text-right text-gray-600 dark:text-gray-400">{{ formatPrice(row.quantity * row.cost_price) }}</td>
                  <td class="px-2 py-2">
            ...
            <div class="flex items-center justify-between border-t border-gray-200 dark:border-gray-700 pt-4">
              ...
              <div class="flex gap-2">
                <button type="button" class="px-3 py-2 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-md" @click="showCreateModal = false">Cancel</button>
    ...
    <template v-else>
      ...
      <p v-if="detailLoading" class="text-gray-600 dark:text-gray-400">Loading...</p>
      <p v-else-if="detailError" class="text-red-600">{{ detailError }}</p>
      <template v-else-if="detail">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-gray-200 mb-4">Purchase detail</h1>
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow border border-gray-200 dark:border-gray-800 p-4 mb-6">
          <dl class="grid grid-cols-2 gap-2 text-sm">
            <dt class="text-gray-500 dark:text-gray-400">Supplier</dt>
            <dd class="text-gray-800 dark:text-gray-200">{{ detail.purchase?.supplier_name || '—' }}</dd>
            <dt class="text-gray-500 dark:text-gray-400">Invoice</dt>
            <dd class="text-gray-800 dark:text-gray-200">{{ detail.purchase?.invoice_number || '—' }}</dd>
            <dt class="text-gray-500 dark:text-gray-400">Date</dt>
            <dd class="text-gray-800 dark:text-gray-200">{{ formatDate(detail.purchase?.created_at) }}</dd>
            <dt class="text-gray-500 dark:text-gray-400">Total</dt>
            <dd class="text-gray-800 dark:text-gray-200 font-medium">{{ formatPrice(detail.purchase?.total_amount) }}</dd>
            <template v-if="detail.purchase?.notes">
              <dt class="text-gray-500 dark:text-gray-400">Keterangan</dt>
              <dd class="text-gray-800 dark:text-gray-200">{{ detail.purchase.notes }}</dd>
            </template>
          </dl>
        </div>
