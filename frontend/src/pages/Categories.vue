<template>
  <div>
    ...
    <div v-else class="bg-white rounded-lg shadow border border-gray-200 overflow-hidden">
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
          <tr>
            <th scope="col" class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Category Name</th>
            <th scope="col" class="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">Action</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-200">
          <tr v-for="c in categories" :key="catId(c)" class="hover:bg-gray-50">
            <td class="px-4 py-2 text-sm text-gray-800">{{ catName(c) }}</td>
          ...
          <tr v-if="!categories?.length">
            <td colspan="2" class="px-4 py-4 text-sm text-gray-500 text-center">No categories yet. Add one above.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    >
      <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm">
        ...
        <form @submit.prevent="handleCreateCategory">
          <div class="mb-4">
            <label for="add-cat-name" class="block text-sm font-medium text-gray-700 mb-1">Category name</label>
            <input
              id="add-cat-name"
          ...
          <div class="flex gap-2 justify-end">
            <button type="button" class="px-3 py-2 text-gray-600 hover:bg-gray-100 rounded-md" @click="showAddModal = false">Cancel</button>
    ...
    >
      <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm">
        ...
        <form @submit.prevent="handleUpdateCategory">
          <div class="mb-4">
            <label for="edit-cat-name" class="block text-sm font-medium text-gray-700 mb-1">Category name</label>
            <input
              id="edit-cat-name"
          ...
          <div class="flex gap-2 justify-end">
            <button type="button" class="px-3 py-2 text-gray-600 hover:bg-gray-100 rounded-md" @click="showEditModal = false">Cancel</button>
    ...
    >
      <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm">
        <h2 class="text-lg font-semibold text-gray-800 mb-2">Delete Category</h2>
        <p class="text-gray-600 mb-4">Delete this category?</p>
        <p v-if="deleteCategoryName" class="text-sm font-medium text-gray-800 mb-4">"{{ deleteCategoryName }}"</p>
